...
for (col in cpi_cols) {
  ...
  # ---------------------------------------------------
  # Table 1: Observed Series Summary
  # ---------------------------------------------------
  ...
  table1_list[[col]] <- data.frame(
    CPI    = col,
    Metric = c("Start CPI (2020-01)", "End CPI (2025-12)",
               "Total Increase", "Increase Rate"),
  ...
  seasonal_ratio <- seasonal_range / series_mean   # additive: normalise by mean
